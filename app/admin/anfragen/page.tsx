...
type Ref<T> = T | T[] | null;
type RequestRow = { id: string; request_type: "reorder" | "suggestion"; title: string; note: string | null; status: "open" | "handled"; created_at: string; created_by: string; location: Ref<{ name: string }>; wine: Ref<{ name: string; producer: string | null }> };
...
export default function AdminRequestsPage() {
  ...
  async function loadRequests() {
    const [requestResult, profileResult] = await Promise.all([
      supabase.from("purchase_requests").select("id,request_type,title,note,status,created_at,created_by,location:locations(name),wine:wines(name,producer)").order("created_at", { ascending: false }),
