...
export default function BetriebPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const ownOrders = await supabase.from("orders").select("id,status,created_at,delivery_date,order_items(cartons,unit_price,wine:wines(name,purchase_price))").eq("location_id", ownProfile.location_id).order("created_at", { ascending: false });
      if (!mounted) return;
  ...
  async function submitOrder() {
    ...
    const itemError = await supabase.from("order_items").insert(Object.entries(cart).map(([wineId, cartons]) => ({ order_id: order.id, wine_id: wineId, cartons, unit_price: wines.find((wine) => wine.id === wineId)?.purchase_price ?? 0 })));
    ...
    const refreshed = await supabase.from("orders").select("id,status,created_at,delivery_date,order_items(cartons,unit_price,wine:wines(name,purchase_price))").eq("location_id", profile.location_id).order("created_at", { ascending: false });
