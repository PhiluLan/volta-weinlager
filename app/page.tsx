...
export default function Home() {
  ...
  const [masterOriginCountry, setMasterOriginCountry] = useState("");
  const [masterGrapeVarieties, setMasterGrapeVarieties] = useState("");
  ...
  useEffect(() => {
    ...
    let mounted = true;
    Promise.all([
      supabase.from("wines").select("id,name,producer,vintage,category,origin_country,grape_varieties,purchase_price,cartons_per_case,min_stock").eq("active", true).order("name"),
      ...
      setInventory((wineResult.data ?? []).map((wine) => ({ id: wine.id, name: wine.name, producer: wine.producer ?? "", vintage: wine.vintage ?? "–", category: wine.category, originCountry: wine.origin_country ?? "", grapeVarieties: wine.grape_varieties ?? "", stock: balances[wine.id] ?? 0, minStock: wine.min_stock ?? 10, purchasePrice: wine.purchase_price ?? 0, cartonsPerCase: wine.cartons_per_case ?? 6 })));
  ...
  function editMasterWine(wine: Wine) {
    setMasterWineId(wine.id ?? null); setMasterName(wine.name); setMasterProducer(wine.producer); setMasterCategory(wine.category); setMasterVintage(wine.vintage === "–" ? "" : wine.vintage); setMasterOriginCountry(wine.originCountry ?? ""); setMasterGrapeVarieties(wine.grapeVarieties ?? ""); setMasterBottleSize(""); setMasterCartonsPerCase(String(wine.cartonsPerCase ?? 6)); setMasterPurchasePrice(String(wine.purchasePrice ?? "")); setMasterMinStock(String(wine.minStock));
  }
  function resetMasterWine() { setMasterWineId(null); setMasterName(""); setMasterProducer(""); setMasterCategory(""); setMasterVintage(""); setMasterOriginCountry(""); setMasterGrapeVarieties(""); setMasterBottleSize(""); setMasterCartonsPerCase("6"); setMasterPurchasePrice(""); setMasterMinStock("10"); }
  async function saveMasterWine() {
    ...
    const values = { name, producer: masterProducer.trim() || null, category, vintage: masterVintage.trim() || null, origin_country: masterOriginCountry.trim() || null, grape_varieties: masterGrapeVarieties.trim() || null, bottle_size_l: bottleSize, cartons_per_case: cartonsPerCase, purchase_price: purchasePrice, min_stock: minStock, active: true };
    ...
    if (masterWineId) setInventory((items) => items.map((item) => item.id === masterWineId ? { ...item, ...values, producer: values.producer ?? "", vintage: values.vintage ?? "–", originCountry: values.origin_country ?? "", grapeVarieties: values.grape_varieties ?? "", purchasePrice: values.purchase_price, cartonsPerCase: values.cartons_per_case, minStock: values.min_stock } : item));
    else setInventory((items) => [...items, { id: result.data.id, name, producer: values.producer ?? "", category, vintage: values.vintage ?? "–", originCountry: values.origin_country ?? "", grapeVarieties: values.grape_varieties ?? "", stock: 0, minStock, purchasePrice, cartonsPerCase }].sort((a, b) => a.name.localeCompare(b.name)));
  ...
  if (active === "Stammdaten") {
    return <MasterDataView inventory={inventory} query={query} setQuery={setQuery} selectedId={masterWineId} name={masterName} producer={masterProducer} category={masterCategory} vintage={masterVintage} originCountry={masterOriginCountry} grapeVarieties={masterGrapeVarieties} bottleSize={masterBottleSize} cartonsPerCase={masterCartonsPerCase} purchasePrice={masterPurchasePrice} minStock={masterMinStock} setName={setMasterName} setProducer={setMasterProducer} setCategory={setMasterCategory} setVintage={setMasterVintage} setOriginCountry={setMasterOriginCountry} setGrapeVarieties={setMasterGrapeVarieties} setBottleSize={setMasterBottleSize} setCartonsPerCase={setMasterCartonsPerCase} setPurchasePrice={setMasterPurchasePrice} setMinStock={setMasterMinStock} onSelect={editMasterWine} onReset={resetMasterWine} onSave={saveMasterWine} saving={masterSaving} onBack={() => setActive("Übersicht")} />;
  }
  if (active === "Stammdaten") {
  ...
  if (active === "Benutzer") { router.push("/admin/benutzer"); return <main className="auth-page"><div className="auth-card"><div className="brand-mark">VB</div><h1>Benutzerverwaltung wird geöffnet …</h1></div></main>; }
  if (active === "Bestellungen") { router.push("/admin/bestellungen"); return <main className="auth-page"><div className="auth-card"><div className="brand-mark">VB</div><h1>Bestellungen werden geöffnet …</h1></div></main>;
  }
  if (active === "Bestellungen-alt") {
